const {
    ...
    testXConnection,
} = require("../services/platformPublishers");
...
async function list(req, res, next) {
    try {
        ...
        return res.json({ success: true, data: [
            ...
            { platform: "X", enabled: configured.includes("X"), configured: Boolean(process.env.X_API_KEY && process.env.X_API_SECRET && process.env.X_ACCESS_TOKEN && process.env.X_ACCESS_TOKEN_SECRET) },
        ]});
...
async function testX(req, res, next) {
    try {
        return res.json({
            success: true,
            message: "X account connection is working",
            data: await testXConnection(),
        });
    } catch (error) {
        next(error);
    }
